...
const CartSummary = ({total,loggedInUserEmail}:{total:number,loggedInUserEmail:string | null | undefined}) => {
  ...
  async function handleInitiatePayment() {
    ...
    try{
      ...
      // Paystack returns a URL from your Django backend. 
      // We need to redirect the browser to this URL.
      if (response && response.data && response.data.checkout_url) {
          window.location.href = response.data.checkout_url;
      } else {
          console.error("Paystack URL not found in response", response);
          // Optional: handle error state here (e.g., show a toast notification)
      }
    }
  ...
  return (
    ...
    <div className="w-full flex items-center justify-between py-2">
      ...
      <p className="text-gray-800 font-semibold">Ksh {formattedSubtotal}</p>
    </div>

    <div className="w-full flex items-center justify-between py-2">
      <p className="text-gray-500 font-medium">Estimated Tax</p>
      <p className="text-gray-800 font-semibold">Ksh {formattedTax}</p>
    </div>
    ...
    <div className="w-full flex items-center justify-between py-2">
      ...
      <p className="text-lg font-bold text-black">Ksh {cart_total}</p>
    </div>
